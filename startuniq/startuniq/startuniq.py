...
"""
__version__ = 'v0.0.5 2026-08-12'# Do not ssh on localhost
...
class KillStartDialog:
    """A simple dialog to ask the user whether to kill a running process or start it anyway."""
...
def run_ssh(host: str, remote_cmd: str) -> subprocess.CompletedProcess[str]:
    ...
    cmd = shlex.split(remote_cmd) if host == 'localhost'\
        else ["ssh", host, remote_cmd]
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Start remote program uniquely",
        ...
        epilog=__version__)
    #parser.add_argument("-S", "--sleep", default=0.1, type=float, help='delay')
    parser.add_argument("-s", "--server", default='localhost', help="Remote host")
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Run the check command on the remote or local host
    ...
    instances = int(stdout) if stdout.isdigit() else 0
    if instances > 1:
        if instances == 2:
            txt = (f"'{process_name}' seem to be running on {host}.\n\n"
            "Press Kill to terminate it, or Start to start it.")
        else:
            txt = (f"{instances-1} instances of '{process_name}' seem to be running on {host}.\n\n"
            "Press Kill to terminate them all, or Start to start new instance.")
    ...
    launchCmd = " ".join(shlex.quote(part) for part in program_argv)
    cmd = shlex.split(launchCmd) if host == 'localhost' else ["ssh", host, launchCmd]
    print(f"Starting command: {cmd}")
    r = subprocess.Popen(cmd, text=True,)
    #print(f"Start result: returncode={r.returncode}, stdout={r.stdout.strip()}, stderr={r.stderr.strip()}")
    #print(f"Start result: returncode={r.returncode}")
